...
### 14:36 修复 session — 多余div闭合

#### 问题
- 时间选择select无法操作（点不了）

#### 根因
- 移除flex包裹层后，`</div></div></div>` 多了一个
- HTML结构被破坏，form-row之外出现游离`</div>`，浏览器解析异常

#### 修复
- 结束时间select后的3个`</div>`改为2个（关time-field + 关form-row）

#### 提交
- `7aaa7ea` fix: 多余div闭合破坏select渲染 — 结束时区后面3个改为2个

### 14:40 暂停（用户反馈继续fix）

### 14:47 日期和时合并到一行 — 修复备注框高度

**问题：** 日期一行、开始/结束一行占两行，备注框被挤压。

**方案：** 一个 form-row 放三列：日期（flex:2.5）+ 开始（flex:1）+ 结束（flex:1），不占额外空间。
...
**方案（一步到位不二次改）：**
- 基础字号 1rem = 16px → 18px（+12.5%）
- 最小字号钳制 13px（原 9.6px，+35%）
- Tab 按钮字号 10px → 13.5px（+35%）
- 灰色文字对比度从 4.8:1 提至 7.8:1

**提交：** `66c5546`

### 17:47-18:06 讨论 App 化 +

**讨论**
1. PWA 增强 vs Capacitor（$99/年）vs 免费自签
2. 字太小问题（根因分析和整体提升方案）
3. 离线缓存、推送通知、捷径推迟
4. 备用方案：不花钱也能做不少事（Service Worker、Web Push）

**结论：**
- 核心功能已可用，日常使用 OK
- 推迟项：离线缓存、推送通知、iOS 捷径、分享功能
- 待完善项：电脑端细节差异、未来规划模块、快速添加按钮

### 18:06 用户要求总结，下一步开新进程

### 19:30-20:00 电脑端优化 + 离线缓存 session

**电脑端优化：**
- 侧栏导航从无效果的占位 → 实际切换内容视图（概览/日历/项目/目标/库）
- 新增完整日历页（可切换日期查看事件）
- 新增完整项目页（卡片网格布局）
- 新增完整目标页（长短期分两列）
- 新增完整库页（条目列表+类型筛选）
