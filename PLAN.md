...
### 数据模块
- [x] **日历/今日待办** — 每日事件列表、勾选完成、本周概览
- [x] **项目列表** — 名称、状态徽章、进度条、标签
- [x] **目标** — 长短期目标、进度、截止日期
- [x] **思考库/行动库/规划库** — 统一 timeline，类型筛选

### 交互
- [x] 手机底部 Tab 导航（今日/日历/项目/目标/库）
- [x] 电脑左侧边栏 + 三列仪表盘
...
| end_time | TIME | 结束时间 |
| event_type | TEXT | 类型：task/appointment/reminder |
...
### 需要修复的问题
- [ ] 手机端 Safari 的渲染和交互问题（用户反馈了）
- [ ] 电脑端与 mockup-desktop 的细节差异（用户反馈了）
- [ ] 整个应用需要全面测试，不应该是用户来报 bug

### 需要完善的功能
- [ ] 添加「编辑」功能（目前只能新建和勾选，无法编辑已有条目）
- [ ] 删除功能（目前没有删除按钮）
- [ ] 未来规划模块（时间轴视图）
- [ ] PWA 配置（添加到主屏幕）
